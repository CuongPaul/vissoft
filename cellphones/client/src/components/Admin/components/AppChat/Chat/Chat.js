...
function Chat(props) {
    ...
    useEffect(() => {
        ...
        const getAllMessageByConversation = async () => {
            const { data } = await axios.get(
                `http://localhost:5000/chat/message?conversationID=${conversationID}`
            );
            console.log(data);
    ...
    useEffect(() => {
        ...
        socket.emit('admin_join_conversation', conversationID);
    ...
    return (
        <div className="ad-chatuser">
            <div className="ad-chatuser-user">
                <span className="ad-chatuser-user-name">{name}</span>
            </div>
