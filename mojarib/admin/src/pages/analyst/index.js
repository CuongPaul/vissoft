...
import {deleteAnalyst, getAnalystList, getMoreAnalyst} from "../../../firebase/analyst";
...
export default function Index() {
    ...
    const loadMore = useCallback(() => {
        ...
        getMoreAnalyst(lastVisible).then(resData => {
                        ...
                        {list.map((row) => (
                            ...
                            >
                                ...
                                <TableCell align="right"><Badge
                                    className={`badge ${row.active ? 'text-success bg-success-light' : 'text-danger bg-danger-light'}`}>{row.active ? 'Active' : 'Deactive'}</Badge></TableCell>
                                <TableCell
                                    align="right">{`${row.createdAt.toDate().getDate()}/${row.createdAt.toDate().getMonth() + 1}/${row.createdAt.toDate().getFullYear()}`}</TableCell>
            ...
            >
                ...
                <DialogContent>
                    <DialogContentText id="alert-dialog-description">
                        Are you sure you want to delete this analyst. This can't be undone ?
                    </DialogContentText>
                </DialogContent>
                <DialogActions>
